...
def sql_to_json_processor(src_path: str, out_path: str, file_name: str) -> None:
    ...
    analyzer = JSONTOPLJSON(sql_content)
...
def read_oracle_triggers_to_json() -> None:
    ...
    for file in os.listdir("files/format_json"):
        if file.endswith(".json"):
            file_name = file.split(".")[0].replace("_analysis", "")
            print(f"Processing file: {file_name}")
            sql_to_json_processor(f"files/format_json/{file}", f"files/format_plsql/{file_name}_postgresql.json", file_name)
