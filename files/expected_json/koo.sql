...
-- Generated on: 2025-08-14 12:45:23
...
BEGIN
  ...
  SELECT NEW_RG_NO INTO V_NEW_RG_NO FROM ( SELECT NEW_RG_NO FROM ( SELECT ROWNUM AS NEW_RG_NO  /* CONNECT BY - replace with recursive CTE */ 1 = 1 AND ROWNUM <= 6999 ) WHERE NEW_RG_NO > 5999 MINUS SELECT TO_NUMBER(RG_NO) FROM V_THEME_MOLECULES ) WHERE ROWNUM = 1;
  ...
  ELSIF TRUE THEN
    IF ((:OLD.IN_PREP_IND = 'N') AND V_IS_ADMIN_CNT = 0) THEN
      RAISE EXCEPTION 'MDM_V_THEMES_IOF: Admin access required for this operation';
    END IF;
    IF (TRUNC(TO_DATE(:OLD.REGISTRAT_DATE, 'DD-MM-YYYY')) = TRUNC(CURRENT_TIMESTAMP)) THEN
  ...
  EXCEPTION
    WHEN ADMIN_UPDATE_ONLY THEN
      RAISE EXCEPTION 'Error -20115: %', 'MDM_V_THEMES_IOF';
    WHEN IN_PREP_NOT_PORTF_PROJ THEN
      RAISE EXCEPTION 'Error -20116: %', 'MDM_V_THEMES_IOF';
    WHEN IN_PREP_NOT_CLOSED THEN
      RAISE EXCEPTION 'Error -20117: %', 'MDM_V_THEMES_IOF';
    WHEN INVALID_MOLECULE_ID THEN
      RAISE EXCEPTION 'Error -20118: %', 'This is not a valid Molecule ID';
    WHEN SEC_MOL_LIST_NOT_EMPTY THEN
      RAISE EXCEPTION 'Error -20119: %', 'MDM_V_THEMES_IOF';
    WHEN INVALID_THEME_NO THEN
      RAISE EXCEPTION 'Error -20101: %', 'This is not a valid Theme No';
    WHEN DELETE_NO_MORE_POSSIBLE THEN
      RAISE EXCEPTION 'Error -20400: %', 'Theme cannot be deleted when the deletion is not on the same day, on which the Theme has been inserted';
    WHEN THEME_NO_ONLY_INSERT THEN
      RAISE EXCEPTION 'Error -20400: %', 'Theme No cannot be updated';
    WHEN DESCRIPTION_TOO_LONG THEN
      RAISE EXCEPTION 'Error -20400: %', 'The automatically generated Theme Description \"' || V_DESCRIPTION || '\" is too long';
    WHEN THEME_DESC_PROPOSAL_TOO_LONG THEN
      RAISE EXCEPTION 'Error -20400: %', 'The automatically generated Short Description Proposal \"' || :OLD.THEME_DESC_PROPOSAL || '\" is too long';
    WHEN THEME_DESC_ALT_TOO_LONG THEN
      RAISE EXCEPTION 'Error -20400: %', 'The automatically generated Downstream Theme Description \"' || :OLD.THEME_DESC_ALT || '\" is too long';
    WHEN THEME_NO_CANNOT_BE_INSERTED THEN
      RAISE EXCEPTION 'Error -20400: %', 'This Theme No already exists';
    WHEN ONLYONEOFFICIALCHANGEPERDAY THEN
      RAISE EXCEPTION 'Error -20400: %', 'Official Change for this Theme No and Day already exists';
    WHEN INSERTSMUSTBEOFFICIAL THEN
      RAISE EXCEPTION 'Error -20400: %', 'New Themes can only be inserted by Official Changes';
    WHEN THEMEDESCRIPTIONMANDATORY THEN
      RAISE EXCEPTION 'Error -20400: %', 'If Pharma Rx Portfolio Project is set to \"No\", then the Theme Description must be filled';
    WHEN THEME_DESC_NOT_UNIQUE THEN
      RAISE EXCEPTION 'Error -20400: %', 'This Theme Description already exists';
    WHEN PORTF_PROJ_MOL_CRE_ERR THEN
      RAISE EXCEPTION 'Error -20120: %', 'MDM_V_THEMES_IOF';
    WHEN DEBUGGING THEN
      RAISE EXCEPTION 'Error -20900: %', 'Debug in Themes IOF standard';
END;
