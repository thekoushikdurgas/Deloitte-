...
BEGIN
  SELECT COALESCE(TXO_SECURITY.GET_USERID, current_user) INTO V_USERID FROM DUAL;
SELECT COUNT(*) INTO V_IS_ADMIN_CNT FROM TXO_USERS_ROLES_MAP WHERE ROLE_ID IN (315) AND USERID = V_USERID;
SELECT NEW_RG_NO INTO V_NEW_RG_NO FROM ( SELECT NEW_RG_NO FROM ( SELECT ROWNUM AS NEW_RG_NO FROM DUAL CONNECT BY 1 = 1 AND ROWNUM <= 6999 ) WHERE NEW_RG_NO > 5999 MINUS SELECT CAST(RG_NO) FROM V_THEME_MOLECULES ) WHERE ROWNUM = 1;
...
END IF;
END IF;
V_ODG_NO := SUBSTRING(:NEW.RESLIN_DESC_CONCAT, 1, 2);
V_RESGRP_CD := SUBSTRING(:NEW.RESLIN_DESC_CONCAT, 4, 2);
V_RESLIN_CD := SUBSTRING(:NEW.RESLIN_DESC_CONCAT, 7, 2);
V_RESLIN_DESC := SUBSTRING(:NEW.RESLIN_DESC_CONCAT, 12, LENGTH(:NEW.RESLIN_DESC_CONCAT));
...
END IF;
V_COUNTER := NULL;
V_VALID_TO := TO_TIMESTAMP('09.09.9999', 'DD.MM.YYYY');
V_SHORT_NAME := COALESCE(:NEW.MANUAL_SHORT_DESC, SUBSTRING(V_DESCRIPTION, 1, 30));
...
END IF;
IF  (:NEW.OFFICIAL_IND = 'N')  THEN
      V_D_REGISTRAT_DATE := TO_TIMESTAMP(:OLD.REGISTRAT_DATE, 'dd-mm-yyyy');
ELSE
      V_D_REGISTRAT_DATE := CURRENT_TIMESTAMP;
END IF;
