...
BEGIN
  ...
  RAISE IN_PREP_NOT_PORTF_PROJ; END IF;
  RAISE IN_PREP_NOT_CLOSED; END IF;
  ...
  RAISE THEME_DESC_PROPOSAL_TOO_LONG; END IF;
  V_SHORT_NAME := COALESCE(:NEW.MANUAL_SHORT_DESC, V_THEME_DESC_PROPOSAL);
  RAISE ADMIN_UPDATE_ONLY; END IF;
  V_MOLECULE_ID := :NEW.MOLECULE_ID;
  RAISE PORTF_PROJ_MOL_CRE_ERR; ELSE INSERT INTO MDM_V_THEME_MOLECULES_MTN( MOLECULE_DESC, VALID_IND, RG_NO, CANCER_IMMUNOTHERAPY_IND, MOLECULE_TYPE_ID, PHARMACOLOGICAL_TYPE_ID ) VALUES ( NVL(:NEW.MANUAL_SHORT_DESC, :NEW.THEME_DESC_PROPOSAL), 'Y', V_NEW_RG_NO, 'N', C_MOLECULE_TYPE_ID, C_PHARMACOLOGICAL_TYPE_ID );
  ...
  RAISE INVALID_THEME_NO; END IF;
  RAISE INVALID_THEME_NO; END IF;
  RAISE INVALID_THEME_NO; END CASE;
  V_COUNTER := NULL;
  ...
  RAISE THEME_NO_CANNOT_BE_INSERTED; END IF;
  V_COUNTER := NULL;
  V_D_REGISTRAT_DATE := SYSDATE;
  RAISE INSERTSMUSTBEOFFICIAL; END IF;
  RAISE THEMEDESCRIPTIONMANDATORY; END IF;
  ...
  RAISE DESCRIPTION_TOO_LONG; END IF;
  ...
  RAISE THEME_DESC_NOT_UNIQUE; END IF;
  V_COUNTER := NULL;
  ...
  RAISE ONLYONEOFFICIALCHANGEPERDAY; END IF;
  V_COUNTER := NULL;
  ...
  RAISE SEC_MOL_LIST_NOT_EMPTY; ELSE UPDATE MDM_V_THEME_MOLECULE_MAP A SET A.VALID_IND = 'N' WHERE A.MOLECULE_ID = :OLD.MOLECULE_ID AND A.THEME_NO = :NEW.THEME_NO AND A.MOLECULE_SEQ_NO = 1 AND A.VALID_IND = 'Y';
  RAISE ADMIN_UPDATE_ONLY; END IF;
  DELETE FROM GMD.THEMES A WHERE A.THEME_NO = :OLD.THEME_NO AND TRUNC(A.REGISTRAT_DATE) = TRUNC(SYSDATE);
  RAISE DELETE_NO_MORE_POSSIBLE; END IF;
  ...
  SELECT A.THEME_NO, A.REGISTRAT_DATE FROM V_THEMES A WHERE A.VALID_TO <= TRUNC(SYSDATE) ) LOOP DELETE FROM GMD.THEMES WHERE THEME_NO = I1.THEME_NO AND REGISTRAT_DATE = I1.REGISTRAT_DATE;
  EXCEPTION
