...
BEGIN
  ...
  EXCEPTION
    WHEN ERR_UPD THEN
      RAISE EXCEPTION 'Error -20111: %', 'The address cannot be updated because the Address type is different. Old address type: ' || :OLD.ADDRESS_TYPE_CD || ' New address type: ' || :NEW.ADDRESS_TYPE_CD;
    WHEN ERR_INS THEN
      RAISE EXCEPTION 'Error -20112: %', 'An address already exists for this Company and Address type. To modify the existing address, please use the Update button.';
    WHEN ERR_CTRY_CHG THEN
      RAISE EXCEPTION 'Error -20113: %', 'The company country modified but not the Valid From Date. Please update also the Valid From Date.';
    WHEN TEST_ERR THEN
      RAISE EXCEPTION 'Error -20113: %', 'New: ' || :NEW.COMPANY_CD || ' Old:' || :OLD.COMPANY_CD || 'Count: ' || CNTR;
    WHEN ERR_INS_LEGAL_ADDR THEN
      RAISE EXCEPTION 'Error -20113: %', 'The legal address cannot be inserted for this type of company';
    WHEN ERR_NOT_ALLOWED_TO_INVALIDATE THEN
      RAISE EXCEPTION 'Error -20113: %', 'It is not allowed to invalidate/delete this type of address';
END;
