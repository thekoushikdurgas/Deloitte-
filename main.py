...
def sql_to_json_processor(src_path: str, out_path: str, file_name: str) -> None:
    ...
    logger.debug(f"Reading SQL file: {src_path}")
    try:
...
def convert_postgresql_format_to_sql(
    ...
    try:
        ...
        if "on_insert" in postgresql_data:
            ...
            for insert in postgresql_data["on_insert"]:
                if insert.get("type") == "sql":
                    sql_lines.append(insert["sql"])
            sql_lines.append("")
        ...
        if "on_update" in postgresql_data:
            ...
            for update in postgresql_data["on_update"]:
                if update.get("type") == "sql":
                    sql_lines.append(update["sql"])
            sql_lines.append("")
        ...
        if "on_delete" in postgresql_data:
            ...
            for delete in postgresql_data["on_delete"]:
                if delete.get("type") == "sql":
                    sql_lines.append(delete["sql"])
            sql_lines.append("")
...
def main() -> None:
    ...
    try:
        ...
        # clean the rest_list.csv file
        pd.DataFrame(columns=["filename", "line", "line_no"]).to_csv("utilities/rest_list.csv",mode='w',index=False)
